...
namespace Ae.Dns.Client
{
    ...
    /// </summary>
    public sealed class CustomDnsHttpClient : IDnsClient
    {
        /// <summary>
        /// Controls whether DNS resolves internal addresses.
        /// </summary>
        public bool IsInInternalNet = false;
        ...
        public CustomDnsHttpClient(IOptions<DnsHttpClientOptions> options, HttpClient httpClient)
        {
        ...
        /// Create a new DNS HTTP client using the specified <see cref="DnsHttpClientOptions"/> and <see cref="HttpClient"/> instance.
        /// </summary>
        public CustomDnsHttpClient(DnsHttpClientOptions options, HttpClient httpClient) : this(Options.Create(options), httpClient)
        {
        }
        ...
        /// </summary>
        public CustomDnsHttpClient(HttpClient httpClient) : this(new DnsHttpClientOptions(), httpClient)
        {
        }
        ...
        public async Task<DnsMessage> Query(DnsMessage query, CancellationToken token)
        {
            ...
            if (!query.Header.Host.EndsWith("labserver.internal") || !query.Header.QueryType.Equals(DnsQueryType.A))
                return answer;

            var filteredAnswers = new List<DnsResourceRecord>();

            foreach (var record in answer.Answers)
            {
                var res = (DnsIpAddressResource)record.Resource;

                // Check if the IP address is in the internal network or not
                if (IsInInternalNet && res.IPAddress.AddressFamily == AddressFamily.InterNetwork &&
            ...
            answer.Header.AnswerRecordCount = (short) filteredAnswers.Count;

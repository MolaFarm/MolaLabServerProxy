...
{
    internal class ServiceStartModeChanger
    {
        ...
        {
            string startmode = serviceStartMode.Equals(ServiceStartMode.Manual) ? "demand" : serviceStartMode.ToString().ToLower();
            Run("sc config " + service.ServiceName + " start=" + startmode, out string output, out string error);
            if (output.Contains("错误") || output.Contains("ERROR"))
            {
                throw new Exception("服务启动类型变更失败");
            }
        }
